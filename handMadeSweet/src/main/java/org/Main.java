 ...
 public static MyApp sweetAppSys=new MyApp();
	...
	public static void logInProcess() 
	{
		...
		else 
		{
			...
			{
				sweetAppSys.currentUser.setLoggedIn(true);
	...
	public static void checkUnamePassEmailCity(String password,String userName,String email,String city,Scanner scanner) {
		...
		while(true)
		{
		    ...
		    {
		    	if(sweetAppSys.currentUser.getUserNameFlag()==0)System.out.println("please don't let user name blank! ");
		    	else if (sweetAppSys.currentUser.getUserNameFlag()==1)System.out.println("please try another user name(this is exist in the DB)! ");
		...
		while(true)
		{
		    ...
		    {
		    	if(sweetAppSys.currentUser.getEmailFlag()==0)System.out.println("please don't let user name blank! ");
		    	else if (sweetAppSys.currentUser.getEmailFlag()==1)System.out.println("please try another Email! ");
   ...
   //     scanner.close();	
	...
	public static void shareRecipe() {
		...
		while(true) {
		    ...
		    else if(newRecipe.getUserNameFlag()==1) System.out.println("Please don't let the userName blank!");
	 ...
	 private static void manageAccount(Scanner input) {
		...
		private static void checkCityAndBranch(Scanner input) {
			...
			private static void sendMassage(Scanner input) {
				if(!sweetAppSys.currentUser.getUserRecievedMsgs().isEmpty()) {
				    System.out.println("*****these are all your recived messages");
					for(Massage msg: sweetAppSys.currentUser.getUserRecievedMsgs())System.out.println(msg.toString());}
			            ...
			            if(flag) //correct type
			            {
			            	...
			            	if(flagType)
			            	{
				            	...
				            	if(flagmsg)
				            	{
				            		...
				            		System.out.println(sweetAppSys.currentUser.getUserSentMsgs().toString());
				            		return;
				            	}
				            	else {
				            		sweetAppSys.printStm("Don't let the massage empty!");
			...
			private static void createOwnerSupplierAccountByAdmin(Scanner input) {
				...
				while(true)
				{
				    ...
				    else if(newUser.getUserNameFlag()==0){System.out.println("please don't let user name blank! ");}	    
				    else if(newUser.getUserNameFlag()==1){System.out.println("please try another user name, this is already used! ");}	
				    System.out.println("Enter The User Name of the worker: ");
					System.out.println("or enter 1 to Go Back!");
				}
				...
				while(true)
				{
				    ...
				    else if(newUser.getUserNameFlag()==1){System.out.println("please write valid email format! ");}
				    System.out.println("Enter The Email of the worker: ");
					System.out.println("or enter 1 to Go Back!");
				}
        ...
        while (true) {
            ...
            while (sweetAppSys.currentUser != null && sweetAppSys.currentUser.isLoggedIn()) {
                if (sweetAppSys.currentUser.getRole().equals("User")) {
                    ...
                    if (choice == 8) {
                        sweetAppSys.currentUser.setLoggedIn(false);
