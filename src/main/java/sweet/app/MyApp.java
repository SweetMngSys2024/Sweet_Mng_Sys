...
public class MyApp {
	 ...
	 public MyApp() {
        ...
        users.add(new User("rama", "password675", "Supplier", "Branch 1", "Nablus",  "rama@example.com", "345-678-9012", "Jordan", "1990-06-30"));
        users.add(new User("amal", "password184", "Supplier", "Branch 4", "Jenin", "amal@example.com", "456-789-0123", "Palestine", "1992-08-25"));

        users.add(new User("amal_ahmad", "password1234", "Supplier", "Branch 3", "Jenin", "amal_ahmad@example.com", "567-890-1234", "Jordan", "1995-10-15"));
        ...
        users.add(new User("rama_hasan", "password11", "Supplier", "Branch 2", "Nablus", "rama_hasan@example.com", "890-123-4567", "Palestine", "1991-07-20"));
		...
		products.add(new product("Half Dark Chocolate", "Rich dark chocolate with 50% cocoa", 5.99, 100, 20, 0.1, "Chocolate Supplier A","Branch 3"));
        ...
        sellers.add(new orders("Dark Chocolate", 2, 13.98, "on processing", "aya", "Branch 1", "Nablus"));
        sellers.add(new orders("Half Dark Chocolate", 3, 17.97, "cancelled", "amer", "Branch 3", "Jenin"));
	...
	public boolean showAllOrders() {
		...
		for(orders order: sellers) 
		{
			printStm("User Name:"+order.getUserName()+"  Tot Price:"+order.getPrice()+"  Product name:"+order.getProductName()+"  Qnt:"+order.getQuantity()+"  Status:"+order.getStatus());
		}return true;
	}
	...
	public boolean handleOrderStatus(String status2, String branch) {
		...
		private void handlePendingOrder(orders order) {
	        printStm("Processing pending order for " + order.getUserName()+", and the total price for it: "+order.getPrice());
	    }

	    private void handleDeliveredOrder(orders order) {
	    	printStm("Order delivered to " + order.getUserName()+", and the total price for it: "+order.getPrice());
	    }

	    private void handleProcessingOrder(orders order) {
	    	printStm("Order is being processed for " + order.getUserName()+", and the total price for it: "+order.getPrice());
	    }

	    private void handleCancelledOrder(orders order) {
	    	printStm("Order cancelled for " + order.getUserName()+", and the total price for it: "+order.getPrice());
	    }
	...
	/*	    System.out.println("userRecievedMsgs");
		...
		public String getBranchOwner(String owner) {
			 ...
			 for (User user : users) {
			        if (user.getRole().equals("Owner")|| user.getRole().equals("Supplier")){
